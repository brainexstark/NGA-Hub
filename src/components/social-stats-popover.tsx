            ...
            {!loading && people.map((person) => (
              <div key={person.id} className="flex items-center gap-3 px-5 py-3 hover:bg-white/5 transition-colors">
                <Avatar className="h-10 w-10 border border-white/10 shrink-0">
                  <AvatarImage src={person.profilePicture || `https://picsum.photos/seed/${person.id}/100/100`} className="object-cover" />
                  <AvatarFallback>{(person.displayName || 'U')[0]}</AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-sm text-white truncate">{person.displayName || 'User'}</p>
                ...
                <div className="flex gap-1">
                  ...
                  {type === 'following' ? (
                    <Button size="sm" variant="outline" className="h-8 px-2 rounded-full text-[9px] font-black uppercase border-white/20 hover:text-destructive"
